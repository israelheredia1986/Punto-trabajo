(function(){
  ...
  async function check(){try{}catch(e){console.debug('notification check',e)}}
  ...
  async function page(){shell('notifications',pageTitle('Avisos','Notificaciones de incidencias, tareas, geocercas y control horario.','<button class="btn secondary" onclick="PuntoNotifications.markAll()">Marcar todos como leídos</button>')+'<section class="card"><div id="notificationsPanel"></div></section>');await render()}\n  async function render(){const box=document.querySelector('#notificationsPanel');if(!box)return;try{const rows=await load();box.innerHTML='<div class="section-head"><h2>Avisos</h2><button class="btn secondary" onclick="PuntoNotifications.markAll()">Marcar todos como leídos</button></div>'+(rows.length?rows.map(n=>'<div class="task" style="cursor:pointer;'+(n.read_at?'opacity:.65':'')+'" onclick="PuntoNotifications.markRead(\''+n.id+'\')"><b>'+esc(n.title)+'</b><span class="muted">'+esc(n.body||'')+' · '+new Date(n.created_at).toLocaleString('es-ES')+'</span></div>').join(''):'<div class="empty">No hay avisos.</div>');const badge=document.querySelector('#notificationBadge');if(badge){const u=rows.filter(n=>!n.read_at).length;badge.textContent=u;badge.style.display=u?'inline-flex':'none'}}catch(e){box.innerHTML='<div class="empty">'+esc(e.message||'No se pudieron cargar los avisos.')+'</div>'}}
  function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
  window.PuntoNotifications={load,check,unread,markRead,markAll,render,page};
})();