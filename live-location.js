(function(){
  ...
  function ensureUi(){
    if(!isWorkerView()||document.querySelector('#workerLocationStatus'))return;
    const buttons=document.querySelector('.mobile-actions');
    if(buttons){const node=document.createElement('div');node.id='workerLocationStatus';buttons.parentElement.appendChild(node);}
  }
  async function capture(){
    if(busy||!isWorkerView())return;
    ensureUi();
    const s=Auth.getSession();if(!s?.id)return;
    try{
      ...
      if(previous&&previous.inside!==inside)toast(inside?'Has entrado en una geocerca.':'Has salido de una geocerca.');
