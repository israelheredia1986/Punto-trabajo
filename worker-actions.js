(function(){
  ...
  function updateButtons(entry){
    ...
    if(!mobileActions || !status) return;
    ...
    if(!entry){
      ...
      status.textContent='Fuera de jornada';
    ...
    status.className='status '+(openBreak?'pause':'online');
    status.textContent=openBreak?'En pausa':'En jornada';
